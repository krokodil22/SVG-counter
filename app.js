...
async function loadSvgFile(file){
  ...
  imported.setAttribute('width', String(intrinsic.width));
  imported.setAttribute('height', String(intrinsic.height));
  imported.style.width = `${intrinsic.width}px`;
  imported.style.height = `${intrinsic.height}px`;
  imported.style.maxWidth = 'none';
  imported.style.maxHeight = 'none';
...
function applyFitScale(){
  if (!svgRoot) return;
  const available = Math.max(50, svgHost.clientWidth - 24);
  fitScale = intrinsic.width ? (available / intrinsic.width) : 1;
  svgRoot.style.transform = `scale(${fitScale})`;
}
